...
import { fetchCategories, fetchContextualCategoryIds } from '../services/wordpress.js';
...
export default function News() {
   ...
   * - Catégories contextuelles : élections, événements, patrimoine...
   * Calculées dynamiquement via fetchContextualCategoryIds()
   */
  ...
  useEffect(() => {
    async function loadCategories() {
      try {
        ...
        // - toutes les catégories (pour le dropdown)
        // - IDs à exclure (bannières + contextuelles)
        const [cats, excludedIds] = await Promise.all([
          fetchCategories(),
          fetchContextualCategoryIds(),
        ]);

        const excludedSet = new Set(excludedIds);

        // Catégories normales pour le dropdown (sans les excludes)
        const normalCats = cats.filter(cat => !excludedSet.has(cat.id));
        setCategories(normalCats);

        // Chaîne d'IDs à exclure pour fetchPosts
        const excludeStr = excludedIds.join(',');
        setExcludedCategoryIds(excludeStr);

        logger.log(`[News] Loaded ${normalCats.length} categories, excluded: ${excludeStr}`);
      } catch (err) {
        logger.error('[News] Error loading categories:', err);
        setCategories([]);
        setExcludedCategoryIds(BANNERS_CATEGORY_ID);
      } finally {

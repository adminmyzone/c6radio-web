...
import { WP_ENDPOINTS, MAX_MENU_PAGES, API_TIMEOUT, WP_API_BASE_URL, CONTEXTUAL_CATEGORY_PREFIXES } from '../config/constants.js';
 ...
 * @param {boolean} options.sticky - true = uniquement les articles épinglés ("à la une" dans WP)
...
export async function fetchPosts(options = {}) {
  try {
    ...
    const {
      ...
      sticky = null,  // true = articles épinglés seulement
    } = options;
    ...
    // sticky=true → WordPress ne renvoie que les articles épinglés ("à la une")
    if (sticky !== null) {
      params.append('sticky', sticky.toString());
    }
...
export async function fetchContextualPages(context) {
  try {
    ...
    // NOTE : On n'utilise PAS _fields ici car WordPress peut exclure les données
    // ACF du filtre (elles ne font pas partie des champs natifs REST API).
    const params = new URLSearchParams({
    ...
    // Diagnostic : afficher les valeurs ACF des 3 premières pages
    if (pages.length > 0) {
      logger.log(`[WordPress API] Sample ACF data:`, pages.slice(0, 3).map(p => ({
        slug: p.slug,
        acf: p.acf,
      })));
    }
    ...
    // ACF peut retourner :
    //   - une string  : "elections"              (Return format: Value, single select)
    //   - un tableau  : ["elections"]            (Return format: Value, multiple select)
    //   - un label    : "Élections"              (Return format: Label)
    //   - []          : tableau vide = pas de valeur
    const normalize = (v) =>
      typeof v === 'string'
    ...
    const contextualPages = pages
      .filter(page => {
        const val = page.acf?.context_section;
        if (!val) return false;
        // Cas tableau (multiple select)
        if (Array.isArray(val)) {
          return val.some(v => normalize(v) === context || v === context);
        }
        // Cas string (single select ou label)
        return val === context || normalize(val) === context;
      })
      .map(page => ({
        id: page.id,
        slug: page.slug,
...
/**
 * Fetch les IDs des catégories contextuelles à exclure
 *
 * EXPLICATION :
 * Récupère les catégories WordPress et identifie celles qui appartiennent
 * à des sections contextuelles (Élections, Événements, Patrimoine…) en
 * vérifiant le slug par rapport aux préfixes définis dans CONTEXTUAL_CATEGORY_PREFIXES.
 * Inclut toujours l'ID 32 (bannières).
 *
 * Utilisé par Home.jsx et News.jsx pour exclure ces catégories des listes
 * d'articles généraux.
 *
 * @returns {Promise<number[]>} Tableau d'IDs à exclure
 */
export async function fetchContextualCategoryIds() {
  try {
    const cats = await fetchCategories();

    const BANNERS_ID = 32;
    const excludedIds = new Set([BANNERS_ID]);

    cats.forEach(cat => {
      const isBanner = cat.id === BANNERS_ID ||
                       cat.slug === 'bannieres' ||
                       cat.name.toLowerCase().includes('bannière') ||
                       cat.name.toLowerCase().includes('banniere');

      const isContextual = CONTEXTUAL_CATEGORY_PREFIXES.some(prefix => cat.slug.startsWith(prefix));
    ...
    logger.log(`[WordPress API] Contextual category IDs to exclude: ${result.join(',')}`);
    return result;

  } catch (error) {
    logger.error('[WordPress API] Error fetching contextual category IDs:', error);
    return [32]; // Fallback : exclure au minimum les bannières
  }
}
